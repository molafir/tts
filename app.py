...
# 📊 تابع بررسی طول متن
def validate_text_length(text, max_tokens=32000):
    """بررسی محدودیت طول متن بر اساس توکن"""
    # تخمین تقریبی تعداد توکن (4 کاراکتر ≈ 1 توکن)
    estimated_tokens = len(text) / 4
    return estimated_tokens <= max_tokens, estimated_tokens

# 🎭 تابع تولید رونوشت خودکار
def generate_transcript(client, topic, speaker1="علی", speaker2="سارا", style="پادکست"):
    """تولید خودکار رونوشت با Gemini"""
    prompt = f"""
    یک مکالمه {style} حدود 150 کلمه بین {speaker1} و {speaker2} درباره "{topic}" ایجاد کن.
...
if api_key:
    try:
        ...
        if auto_generate:
            ...
            if st.button("🪄 تولید رونوشت", key="generate_transcript"):
                with st.spinner("در حال تولید رونوشت..."):
                    transcript = generate_transcript(client, topic, style=style)
                    if transcript:
        ...
        # استفاده از رونوشت تولید شده یا ورودی کاربر
        ...
        # 🔊 لیست کامل گزینه‌های صوتی از مستندات
        all_voices = [
        ...
        if mode == "تک‌بلندگو":
            ...
            with col2:
                # اضافه کردن دستورات سبک اختیاری
        ...
        # 📊 بررسی طول متن
        if text_input:
            is_valid, estimated_tokens = validate_text_length(text_input)
        ...
        if st.button("🎧 تولید صدا", type="primary", use_container_width=True):
            if not text_input.strip():
                st.error("❌ لطفاً متن مورد نظر را وارد کنید")
            else:
                try:
                    with st.spinner("🔮 در حال تولید صدا..."):
                        
                        # پردازش متن با دستورات سبک
                        processed_text = text_input
                        
                        if mode == "تک‌بلندگو":
                            # اضافه کردن دستور سبک اگر وجود دارد
                            ...
                            # پردازش دستورات سبک برای چندبلندگو
                            style_prefix = ""
                            if style1 or style2:
                                style_parts = []
                                if style1:
                    ...
                    st.info("💡 ممکن است کلید API نامعتبر باشد یا سرویس در دسترس نباشد")
